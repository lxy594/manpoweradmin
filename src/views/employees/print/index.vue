    ...
    <div class="app-container">
      <el-card>
        <el-breadcrumb separator="/" class="titInfo ">
          ...
          <el-breadcrumb-item v-print="printisObj" size="small" type="primary">打印</el-breadcrumb-item>
        </el-breadcrumb>
        <div v-if="type === 'personal'">
          <h2 class="centInfo">员工信息表</h2>
          <table cellspacing="0" width="100%" class="tableList" border="true">
            <tr class="title">
...
import { getPersonMess, getjobs } from '@/api/employess'
export default {
  data() {
    return {
      printisObj: {
  ...
  created() {
    this.type === 'personal' ? this.getPersonMess() : this.getjobs()
  },
  methods: {
    async getPersonMess() {
    ...
    async getjobs() {
      const userInfo = await getUserheadByid(this.userId)
      const jobInfo = await getjobs(this.userId)
