        ...
        <el-col :span="12">
          <el-form-item label="聘用形式">
            <el-select v-model="userInfo.formOfEmployment" class="inputW">
              <el-option
                v-for="item in employessMju.hireType"
                :key="item.id"
                :label="item.value"
                :value="item.id"
              />
      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <ImageUpload ref="staffPhotohead" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="最高学历">
          ...
          >
            <el-option
              v-for="item in employessMju.highestDegree"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
        ...
        <el-form-item label="国家/地区">
          <el-select v-model="formData.nationalArea" class="inputW2">
            <el-option
              v-for="item in employessMju.isOverseas"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
        ...
        <el-form-item label="婚姻状况">
          <el-select v-model="formData.maritalStatus" class="inputW2">
            <el-option
              v-for="item in employessMju.maritaStatus"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
        ...
        <el-form-item label="星座">
          <el-select v-model="formData.constellation" class="inputW2">
            <el-option
              v-for="item in employessMju.constellation"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
        ...
        <el-form-item label="血型">
          <el-select v-model="formData.bloodType" class="inputW2">
            <el-option
              v-for="item in employessMju.bloodType"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
      ...
      <div class="block">
        ...
        <el-form-item label="学历类型">
          <el-select v-model="formData.educationalType" placeholder="请选择">
            <el-option
              v-for="item in employessMju.educationType"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
...
<script>
// 枚举数据进行
import employessMju from '@/api/constant/employees'
import { saveUserByid, UpdaPersoin, getPersonMess } from '@/api/employess'
...
export default {
  ...
  methods: {
    // 获取员工照片证件信息 api带id所以这里是formData
    async getPersonMess() {
      this.formData = await getPersonMess(this.userId)
      // 不做这一步也可以图片显示出来 但是等下拿图片的话会拿不到---理解决图片没有显示出来------------------
    ...
    // id带userid所以这里是userinfo
    async getUserheadByid() {
      this.userInfo = await getUserheadByid(this.userId)
      console.log(this.userInfo)
      // 员工头像的处理解决图片没有显示出来
      if (this.userInfo.staffPhoto) {
        this.$refs.staffPhotohead.fileList = [
    ...
    async saveUser() {
      // 读取员工上传的头像
      const fileList = this.$refs.staffPhotohead.fileList
      // 上传后面还没成功点保存更新的提示
      ...
      this.$message.success('保存基本信息的成功')
    },
