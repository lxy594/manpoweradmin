      ...
      <div class="block">
        ...
        <el-form-item label="转正状态">
          <el-select v-model="formData.stateOfCorrection" placeholder="请选择" disabled>
            <el-option
              v-for="item in EmployeeMeiju.stateOfCorrection"
              :key="item.value"
              :value="item.value"
            />
          </el-select>
      ...
      <div class="block">
        ...
        <el-form-item label="合同期限">
          <el-select v-model="formData.contractPeriod" class="filter-item">
            <el-option
              v-for="item in EmployeeMeiju.contractPeriod"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
        ...
        <el-form-item label="续签次数">
          <el-select v-model="formData.renewalNumber" class="filter-item">
            <el-option
              v-for="item in EmployeeMeiju.renewalCount"
              :key="item.id"
              :label="item.value"
              :value="item.id"
            />
      ...
      <div class="block">
        ...
        <el-form-item label="其他招聘渠道">
          <el-select v-model="formData.otherRecruitmentChannels" placeholder="请选择">
            <el-option
              v-for="item in EmployeeMeiju.resumeSource"
              :key="item.id"
              :label="item.value"
              :value="item.value"
            />
        ...
        <el-form-item label="社招/校招">
          <el-select v-model="formData.socialRecruitment" placeholder="请选择">
            <el-option
              v-for="item in EmployeeMeiju.hireSourceType"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
...
<script>
// 枚举的组件
import EmployeeMeiju from '@/api/constant/employees'
import { UpdaPersoin, getjobs, getEmployeeSimple } from '@/api/employess'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    // 获取岗位 获取的时候带上他的userid
    async getjobs() {
      this.formData = await getjobs(this.userId)
    },
    //  从业信息的保存更新
    async saveJob() {
      await UpdaPersoin(this.formData)
